...
func (t *PgxZerologTracer) Log(ctx context.Context, level tracelog.LogLevel, msg string, data map[string]any) {
	...
	baseLogger := t.Logger.With().Str("trace_id", logger.GetTraceID(ctx)).Str("sql_original", queryInfo.OriginalSQL).Str("query_name", queryInfo.QueryName).Str("clean_sql", finalSQL).Str("operation_type", queryInfo.OperationType).Any("args", args).Dur("duration", duration).Interface("args", args)
	...
	if msg == "Query" {
 logger.Info().Str("event", "Executed Query").Str("sql", sql).Any("args", args).Dur("duration", duration).Msg("Executed Query")
 return
	}
